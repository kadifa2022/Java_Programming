...
public class CarTask {
    ...
    public static void main(String[] args) {
        ...
        //Array list for recall
        ArrayList<Car> list= new ArrayList<>();
        //for each loop and if statement and instanceof
        for (Car each: cars){
            if(each instanceof Honda) {
                ...
                if(each.getYear() >= 2015 && each.getYear()<=2106){
                    list.add(each);
                }
        ...
        for(Car each: cars){
            priceList.add((int) each.getPrice());//casting
        }
